...
async def import_products(file: bytes = File()):
    ...
    df = df.dropna(subset=[nm_id_column, rrc_column, seller_id_column])
    df = df.drop_duplicates(subset=[nm_id_column, rrc_column, seller_id_column])

    await supplier_services.import_products(
        df=df, nm_id_column=nm_id_column, rrc_column=rrc_column, seller_id_column=seller_id_column)
